...
exports.handler = async function (context, event, callback) {
  const { ACCOUNT_SID, AUTH_TOKEN, TWILIO_FLEX_WORKSPACE_SID, TWILIO_FLEX_RECOVERY_PING_WORKFLOW_SID } =
    context;
  ...
  if (isRelevantPingTaskEvent(context, event)) {
    ...
    if (eventType === 'reservation.accepted') {
      ...
      taskService.updateTask(TWILIO_FLEX_WORKSPACE_SID, taskSid, {
      ...
      await taskService.updateTask(TWILIO_FLEX_WORKSPACE_SID, disconnectedTaskSid, {
  ...
  if (isRelevantVoiceTaskEvent(event)) {
    ...
    if (originalReservation && originalReservation.reservationStatus === "wrapping") {
      ...
      await taskService.updateReservation(TWILIO_FLEX_WORKSPACE_SID, originalTaskSid, originalReservationSid, {
...
function isRelevantPingTaskEvent(context, event) {
  ...
  return (
    workflowSid == context.TWILIO_FLEX_RECOVERY_PING_WORKFLOW_SID &&
