...
export const eventHook = async (_flex: typeof Flex, _manager: Flex.Manager, activity: Activity) => {
  ...
  //
  // We only need the full 3s delay when transitioning INTO a system activity
  // (which indicates a potential outbound call setup). For all other changes
  // (e.g. supervisor changing status), a shorter delay is sufficient and
  // reduces the visible "flip-flop" in the supervisor dashboard.
  const isSystemActivityChange = reservedSystemActivities
    .map((a) => a?.toLowerCase())
    .includes(activity.name.toLowerCase());

  if (isSystemActivityChange) {
    // Outbound call scenario: wait for the task to arrive
    await new Promise((f) => setTimeout(f, 3000));
  } else {
    // Supervisor or non-system activity change: shorter delay to reduce
    // the desync window visible on the supervisor dashboard
